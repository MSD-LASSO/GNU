...
while (i<len(Date)):
    ...
    if currentTime>=Date[i]:
        ...
        if Doppler[i] > 0:
            ...
            top=record_ref_second_device
            tb=top.record_ref_second_device(center_freq=97000000, channel_freq=97900000, file_loc='/home/pi/Documents/Ref_Time'+str(currentTime).replace(" ","_").replace(":","_").replace(".","_"), num_samples=int(round(Length[i]*sampleRate)),
                                samp_rate=sampleRate)
            afterSetup=datetime.now()
            print("After calling class constructor: " + str(afterSetup))
            debuggerFile.write("After calling class constructor: " + str(afterSetup) + '\n')
